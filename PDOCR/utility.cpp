...
std::vector<std::string> Utility::ReadDict(const std::string &path) {
	//const_cast<std::string&>(path) = ".\\ppocr_keys_v1.txt";
...
// 	char *inbuf = const_cast<char*>(strText.c_str());
// 	char *outbuf = new char[strText.length()+1];
// 	code_convert("utf-8", "johab", inbuf, strText.length(), outbuf, strText.length());

	//((CPDOCRDlg*)theApp.GetMainWnd())->DisplayOcrRes(strText);

	std::ofstream outfile(".\\out.txt", std::ios::app);
	outfile << strText;
	outfile.close();

	((CPDOCRDlg*)theApp.GetMainWnd())->m_wndEdit.Clear();
	((CPDOCRDlg*)theApp.GetMainWnd())->m_wndEdit.SetWindowText(UTF82WCS(strText.c_str()));
}
